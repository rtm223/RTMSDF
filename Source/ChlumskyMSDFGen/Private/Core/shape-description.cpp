    ...
    for (std::vector<Contour>::const_iterator contour = shape.contours.begin(); contour != shape.contours.end(); ++contour) {
        ...
        if (!contour->edges.empty()) {
            for (std::vector<EdgeHolder>::const_iterator edge = contour->edges.begin(); edge != contour->edges.end(); ++edge) {
                ...
                if((*edge)->type == EdgeSegment::SegmentType::Linear){
                    const LinearSegment *e = static_cast<const LinearSegment *>(&**edge);
                    fprintf(output, "\t");
                ...
                if((*edge)->type == EdgeSegment::SegmentType::Quadratic){
                    const QuadraticSegment *e = static_cast<const QuadraticSegment *>(&**edge); 
                    fprintf(output, "\t");
                    writeCoord(output, e->p[0]);
                    fprintf(output, ";\n\t\t");
                    if (colorCode)
                ...
                if((*edge)->type == EdgeSegment::SegmentType::Quadratic){
                    const CubicSegment *e = static_cast<const CubicSegment *>(&**edge);
                    fprintf(output, "\t");
                    writeCoord(output, e->p[0]);
                    fprintf(output, ";\n\t\t");
                    if (colorCode)

...
#include <3rdParty/dropXML.hpp>
#endif

#ifdef MSDFGEN_USE_SKIA
#include <3rdParty/skia-simplify.h>
#endif
...
#if PLATFORM_WINDOWS
    // TODO RTM possibly just remove this code entirely from the codebase / strip the import svg code right back to being more sensible
    FILE* f;
...
{
    // NOTE.RTM - Hacked together version of parsing that doesn't require SKIA, for platforms without skia support
    ensureMsgf(false, TEXT("Full SVG parsing is not handled without SKIA, falling back to single shape"));
    int pathIndex = 0;

    SvgPathAggregator pathAggregator;
    if (!dropXML::parse(pathAggregator, &svgData[0], &svgData[0]+svgLength))
        return SVG_IMPORT_FAILURE;
    ...
    viewBox = Shape::Bounds(0,0,dims.x, dims.y);

    output.contours.clear();
    output.inverseYAxis = true;

    // TODO - see if we can munge together multiple shapes here by iterating the paths and just adding them to the array?
    bool success =  buildShapeFromSvgPath(output, xmlDecode(pathAggregator.pathDefs[pathIndex].start, pathAggregator.pathDefs[pathIndex].end).c_str(), ENDPOINT_SNAP_RANGE_PROPORTION*dims.length());
    return success ? SVG_IMPORT_SUCCESS_FLAG : SVG_IMPORT_FAILURE;
}
